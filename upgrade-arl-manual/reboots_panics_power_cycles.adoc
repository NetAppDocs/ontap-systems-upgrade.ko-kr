...
일부 또는 모든 애그리게이트가 node1에서 node2로 재배치되었으며 HA는 여전히 활성화되어 있습니다. Node2는 node1의 루트 볼륨과 재배치되지 않은 루트가 아닌 애그리게이트를 인계받습니다.

.이 작업에 대해
재배치된 애그리게이트의 소유권은 홈 소유자가 변경되지 않았기 때문에 테이크오버된 루트 이외의 애그리게이트의 소유권과 동일하게 표시됩니다. node1이 `waiting for giveback state`에 진입하면 node2는 모든 node1 루트 이외의 애그리게이트를 반환합니다.
...
.이 작업에 대해
Node1은 node2의 모든 애그리게이트와 node2로 재배치했던 자체 애그리게이트를 모두 인수합니다. node2가 `Waiting for Giveback` 상태로 전환되면 node1은 node2의 모든 애그리게이트를 반환합니다.
...
. 노드2를 위로 올립니다.
+
node2가 부팅되는 동안 모든 애그리게이트에 대해 클라이언트 중단이 발생합니다.
...
HA가 이미 비활성화되어 있으므로 node2 충돌 후 node3가 테이크오버를 수행하지 않습니다.
...
Node2는 일부 또는 모든 애그리게이트를 node3로 재배치했으며, node3는 재배치된 애그리게이트의 데이터를 제공합니다. HA가 비활성화되어 있습니다.

.이 작업에 대해
재배치되지 않은 애그리게이트에 대해 클라이언트 운영 중단이 발생합니다.
...
node2가 애그리게이트를 node3으로 재배치하는 동안 node3이 충돌하면 node2는 나머지 애그리게이트의 재배치를 중단합니다.
...
. 노드3을 불러옵니다.
+
모든 애그리게이트에 대해 클라이언트 중단이 발생했습니다.
