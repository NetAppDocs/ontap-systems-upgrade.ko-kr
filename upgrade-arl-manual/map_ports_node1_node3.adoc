...
원래 노드의 포트 및 LIF 구성이 새 노드의 구성 계획과 호환되도록 해야 합니다. 새 노드는 부팅 시 동일한 구성을 재생하기 때문입니다. 즉, node3을 부팅하면 ONTAP는 node1에서 사용했던 것과 동일한 포트에서 LIF를 호스팅하려고 시도합니다.

따라서 node1의 물리적 포트가 node3의 물리적 포트에 직접 매핑되지 않는 경우 부팅 후 클러스터, 관리 및 네트워크 연결을 복원하려면 소프트웨어 구성 변경이 필요합니다. 또한 node1의 클러스터 포트가 node3의 클러스터 포트에 직접 매핑되지 않으면 클러스터 LIF를 올바른 물리적 포트에서 호스팅하도록 소프트웨어 구성을 변경하기 전까지 node3이 재부팅될 때 쿼럼에 자동으로 다시 참여하지 못할 수 있습니다.
...
|===
+
이 정보를 얻는 단계는 link:record_node1_information.html["노드1 정보를 기록합니다"]을(를) 참조하십시오.
...
----
+
link:other-references.html["참조"]로 이동하여 _Network Management_ 또는 _ONTAP 9 Command reference_에 대한 자세한 내용을 확인하십시오.
