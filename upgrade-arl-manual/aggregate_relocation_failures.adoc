...
+
 `storage aggregate relocation start`,  `override-vetoes` 및  `override-destination-checks` 옵션에 대한 자세한 내용은 link:other_references.html["참조"]를 참조하여 _ONTAP 9 명령 참조_에 연결하십시오.
...
* 3단계에서는 애그리게이트가 node2에서 node3으로 재배치됩니다. 재배치되는 애그리게이트 중 일부는 node1을 홈 노드로 사용합니다. 예를 들어, 이러한 애그리게이트를 aggr_node_1이라고 부를 수 있습니다. 3단계에서 aggr_node_1의 재배치가 실패하고 강제 재배치가 불가능한 경우, 해당 애그리게이트는 node2에 그대로 남게 됩니다.
* 4단계 이후, node2가 node4로 교체될 때. node2가 교체되면 aggr_node_1은 node3 대신 node4를 홈 노드로 사용하여 온라인 상태가 됩니다.
