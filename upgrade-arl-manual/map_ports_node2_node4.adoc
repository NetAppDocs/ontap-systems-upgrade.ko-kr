...
[role="lead"]
노드2의 물리적 포트는 노드4의 물리적 포트와 정확하게 매핑되어야 합니다. 그래야 업그레이드 후 노드4가 클러스터의 다른 노드 및 네트워크와 통신할 수 있습니다.
...
.이 작업에 대해
노드 모델에 따라 포트 설정이 다를 수 있습니다. 기존 노드의 포트 및 LIF 구성이 새 노드의 구성과 호환되도록 설정해야 합니다. 새 노드는 부팅 시 동일한 구성을 재생성하기 때문입니다. 즉, 노드4를 부팅하면 Data ONTAP는 노드2에서 사용했던 것과 동일한 포트에서 LIF를 호스팅하려고 시도합니다.

따라서 node2의 물리적 포트가 node4의 물리적 포트에 직접 매핑되지 않는 경우 부팅 후 클러스터, 관리 및 네트워크 연결을 복원하려면 소프트웨어 구성 변경이 필요합니다. 또한 node2의 클러스터 포트가 node4의 클러스터 포트에 직접 매핑되지 않으면 올바른 물리적 포트에서 클러스터 LIF를 호스팅하도록 소프트웨어 구성을 변경하기 전까지는 node4가 재부팅될 때 쿼럼에 자동으로 다시 참여하지 못할 수 있습니다.
...
+
자세한 내용은 link:other-references.html["참조"]를 참조하여 _Network Management_ 또는 _ONTAP 9 Command reference_에 연결하고 _Configuring failover settings on a LIF_로 이동하십시오.
