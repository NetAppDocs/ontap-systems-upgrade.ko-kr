...
+
짝수 개의 노드로 구성된 클러스터에서는 동점이 발생할 가능성이 있으므로, 하나의 노드에는 엡실론(epsilon)이라는 추가적인 소수점 투표 가중치가 부여됩니다. 자세한 내용은 link:other-references.html["참조"]을 참조하여 _System Administration Reference_를 확인하십시오.
...
. 선택적으로, 업그레이드가 완료된 후 node1을 해제합니다. link:decommission_old_system.html["기존 시스템을 폐기합니다"]를 참조하십시오.
