...
summary: 루트가 아닌 애그리게이트와 NAS 데이터 LIF를 노드 1에서 노드 2로 이동하고 AFF A700에서 AFF A900으로 업그레이드 중에 노드 1의 리소스를 복원합니다. 
---
...
[role="lead"]
노드 1을 AFF A900 또는 FAS9500 컨트롤러 및 NVRAM 모듈로 교체하기 전에 노드 1에서 노드 2로 비루트 애그리게이트 및 NAS 데이터 LIF를 이동한 다음, AFF A900 또는 FAS9500 시스템에서 실행되는 노드 1에서 노드 1의 리소스를 다시 복원해야 합니다. 이 프로세스는 대부분 자동화되어 있으며 사용자가 상태를 확인할 수 있도록 작업이 일시 중지됩니다.

작업을 시작할 때 작업이 이미 일시 중지되어 있어야 합니다. 작업을 수동으로 다시 시작해야 합니다.

원격 LIF는 업그레이드 절차 중에 SAN LUN의 트래픽을 처리합니다. 업그레이드 중에 클러스터 또는 서비스 상태를 위해 SAN LIF를 이동할 필요가 없습니다. 노드1을 AFF A900 또는 FAS9500 시스템으로 온라인 상태로 전환하고 LIF가 정상 작동하는지 확인해야 합니다.
...
+
'storage aggregate show -node_node2_ -state online-root false'
...
+
'network interface modify -vserver_vserver_name_-lif_LIF_name_-home-node_nodename_-status-admin up'
