...
summary: HA 쌍을 설정하고 노드 1과 노드 2가 서로의 스토리지에 액세스할 수 있는지 확인하여 올바른 설정을 확인합니다. 
---
...
[role="lead"]
올바른 설정을 위해서는 HA 쌍이 설정되었는지 확인해야 합니다. 또한 노드 1과 노드 2가 서로의 스토리지에 액세스할 수 있고 클러스터의 다른 노드에 속하는 데이터 LIF가 소유하지 않는지 확인해야 합니다. 또한 모든 데이터 애그리게이트가 올바른 홈 노드에 있고 두 노드의 볼륨이 온라인 상태인지 확인해야 합니다. 새 노드 중 하나에 통합 대상 어댑터가 있는 경우 포트 구성을 복원해야 하며 어댑터 사용을 변경해야 할 수도 있습니다.
...
+
에서 캡처한 출력과 출력을 비교할 수 있습니다 link:prepare_nodes_for_upgrade.html["업그레이드할 노드를 준비합니다"] 섹션을 참조하십시오.

. NSE(NetApp Storage Encryption)를 구성에서 사용 중인 경우 'setenv bootarg.storageencryption'을 설정합니다. AFF A900 컨트롤러 모듈 및 NVS_를 사용하여 kmip.init.maxwait` 변수(in_Boot node2에서 변수 'off'로 'true'를 지원하도록 명령을 지원합니다. link:boot_node2_with_a900_controller_and_nvs.html#boot_node2_step1["1단계"]), 변수를 재설정해야 합니다.
+
...
노드 1과 노드 2에서 스토리지 암호화가 설정된 경우 섹션을 완료합니다 link:set_up_storage_encryption_new_module.html["새 컨트롤러 모듈에서 스토리지 암호화를 설정합니다"]. 그렇지 않으면 섹션을 완료합니다 .
