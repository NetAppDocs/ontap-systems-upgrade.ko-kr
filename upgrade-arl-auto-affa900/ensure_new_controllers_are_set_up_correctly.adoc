...
. NSE(NetApp 스토리지 암호화)를 구성에서 사용 중인 경우 를 설정합니다 `setenv bootarg.storageencryption.support` 명령을 사용하여 로 이동합니다 `true` 를 사용하여 `<kmip.init.maxwait>` 변수 `off` (AFF A900 또는 FAS9500 컨트롤러 및 NVRAM 모듈을 사용하는 IN_BOOT 노드2 _, link:boot_node2_with_a900_controller_and_nvs.html#A900_boot_node2["1단계"]), 변수를 재설정해야 합니다.
+
