...
. NSE(NetApp Storage Encryption)를 구성에서 사용 중인 경우 'setenv bootarg.storageencryption.support'를 'kmip.init.maxwait` variable'off'로 설정하여 'true'로 설정하십시오(in_Boot node2에서 AFF A900 컨트롤러 모듈 및 NVRAM 모듈 _, link:boot_node2_with_a900_controller_and_nvs.html#boot_node2_step1["1단계"]), 변수를 재설정해야 합니다.
+
