...
.단계
. 를 설정합니다 `bootarg.storageencryption.support` 및 `kmip.init.maxwait` node1 구성이 로드된 후 부트 루프를 방지하기 위한 변수입니다.
+
...
WARNING: * HA 쌍에서 컨트롤러 업그레이드가 완료되면 즉시 의 설정을 해제해야 합니다 `kmip.init.maxwait` 변수. 을 참조하십시오 link:ensure_new_controllers_are_set_up_correctly.html#unset_maxwait["새 컨트롤러가 올바르게 설정되었는지 확인하고 12단계를 수행합니다"]. 그렇지 않으면 정전 발생 시 데이터가 손실될 수 있습니다.*
...
. "22/7"을 입력하고 숨겨진 옵션을 선택하여 AFF A700 노드 1 디스크를 AFF A900 노드 1에 재할당합니다 `boot_after_controller_replacement` 부팅 메뉴에서 노드가 중지되면
+
