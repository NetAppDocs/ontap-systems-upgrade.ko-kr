...
[role="lead"]
이제 교체 모듈이 있는 노드 2를 부팅할 준비가 되었습니다. 시스템 모듈을 교체하여 업그레이드하는 경우 콘솔과 관리 연결만 이동합니다. 이 섹션에서는 다음 업그레이드 구성에서 노드 2를 교체 모듈로 부팅하는 데 필요한 단계를 설명합니다.

[cols="35,65"]
|===
| 이전 노드2 컨트롤러 | 교체 노드2 시스템 모듈 
