...
.단계
. [[A900_BOOT_node2]] NSE(NetApp Storage Encryption)를 이 구성에서 사용 중인 경우, 'setenv bootarg.storageencryption.support' 명령을 'true'로 설정해야 하며, node2 구성이 로드된 후 부트 루프를 방지하려면 kmip.init.maxwait` 변수를 'off'로 설정해야 합니다.
+
...
. 부팅 메뉴에서 노드가 중지될 때 "22/7"을 입력하고 숨겨진 옵션 "boot_after_controller_replacement"를 선택하여 AFF A700 node2 디스크를 AFF A900 node2에 재할당합니다.
+
