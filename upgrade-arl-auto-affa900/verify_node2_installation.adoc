...
summary: AFF A900 컨트롤러 모듈 및 NVS를 사용하여 node2 설치를 확인합니다. 
---
= 노드2 설치를 확인합니다
:hardbreaks:
:nofooter: 
...
[role="lead"]
AFF A900 컨트롤러 모듈 및 NVS를 사용하여 node2 설치를 확인해야 합니다. 물리적 포트는 변경되지 않으므로 AFF A700 노드 2의 물리적 포트를 AFF A900 노드 2로 매핑할 필요가 없습니다.
...
NOTE: 이 가이드 섹션에서 _VLAN, ifgrp 및 브로드캐스트 도메인 다시 만들기_가 node2_에서 _Restore 네트워크 구성 으로 변경되었습니다.
...
----
+
위 예제에서 노드 2는 컨트롤러 교체 후 쿼럼(quorum)으로 부팅되고 조인되었습니다. 여기에는 연결 기능이 없고 연결 가능성 검사를 대기 중인 여러 포트가 있습니다.
