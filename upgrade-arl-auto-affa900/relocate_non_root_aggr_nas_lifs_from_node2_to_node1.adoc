...
summary: 노드 2를 AFF A900 컨트롤러 모듈 및 NVS로 교체하기 전에 노드 2가 소유한 비루트 애그리게이트를 노드 1로 재이동해야 합니다. 
---
...
[role="lead"]
노드 2를 AFF A900 컨트롤러 모듈 및 NVS로 교체하려면 먼저 노드 2가 소유한 비루트 애그리게이트를 노드 1로 재이동해야 합니다.
