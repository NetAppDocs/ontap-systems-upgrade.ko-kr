...
.이 작업에 대해
교체 시스템 모듈로 노드2를 부팅한 후, 모듈이 올바르게 설치되었는지 확인합니다. 노드2가 쿼럼에 참여할 때까지 기다린 후 컨트롤러 교체 작업을 재개해야 합니다.
...
== 1단계: node2가 쿼럼 상태인지 확인합니다.

node2가 쿼럼 상태인지 확인합니다. 그런 다음 컨트롤러 교체 작업을 재개합니다.
...
. 컨트롤러 교체가 일시 중지된 상태에서 <<restore_network_configuration_on_node2,노드2에서 네트워크 구성을 복원합니다>>로 진행하십시오.
...
.다음 단계
node2 설치를 확인한 후 link:restore-key-manager-config-node2.html["node2에서 key-manager 구성을 복원합니다"].
