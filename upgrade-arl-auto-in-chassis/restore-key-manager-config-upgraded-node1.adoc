...
.다음 단계
업그레이드된 node1에서 키 관리자 구성을 복원한 후 link:move-non-root-aggr-nas-lifs-node1-from-node2-to-upgraded-node1.html["node2에서 업그레이드된 node1로 node1의 루트가 아닌 애그리게이트 및 NAS 데이터 LIF를 이동합니다"].
