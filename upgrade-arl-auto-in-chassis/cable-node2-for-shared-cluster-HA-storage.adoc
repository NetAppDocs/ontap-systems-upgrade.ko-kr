...
summary: 이전에 AFF A800 노드2에 연결되어 있던 클러스터, HA, 스토리지 및 데이터 연결을 새로 설치된 AFF A90 또는 AFF A70 노드2에 연결하십시오. 
---
...
.다음 단계
공유 클러스터 HA 및 스토리지를 위해 node2에 케이블을 연결한 후, link:boot-node2-with-a900-controller-and-nvs.html["교체 시스템 모듈로 node2를 부팅합니다"].
