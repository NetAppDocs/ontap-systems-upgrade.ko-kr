...
교체 컨트롤러 모듈의 부트 미디어에 설치된 ONTAP 버전을 확인하려면 해당 컨트롤러 모듈이 섀시에 설치되어 전원이 켜져 있어야 합니다.  교체 시스템 부팅 미디어의 ONTAP 버전은 업그레이드하는 기존 시스템에서 실행 중인 ONTAP 버전과 동일해야 하며, 부팅 미디어의 기본 부팅 이미지와 백업 부팅 이미지가 모두 일치해야 합니다.  업그레이드에 지원되는 최소 ONTAP 버전을 확인하려면 다음을 참조하세요.link:decide_to_use_the_aggregate_relocation_guide.html#supported-systems-in-chassis["지원되는 시스템 매트릭스"] .
