...
+
 `storage aggregate relocation start`,  `override-vetoes` 및  `override-destination-checks` 옵션에 대한 자세한 내용은 link:other-references.html["참조"]를 참조하여 _ONTAP 9 명령 참조_에 연결하십시오.
...
+
node2가 교체되면 aggr_node_1은 node2 대신 node1을 홈 노드로 사용하여 온라인 상태가 됩니다.
...
+
올바르게 재배치되지 않은 애그리게이트를 식별하려면 link:prepare-nodes-for-upgrade.html["업그레이드할 노드를 준비합니다"] 섹션에서 얻은 node1의 홈 소유자가 포함된 애그리게이트 목록을 참조하여 위 명령의 출력과 비교하십시오.

. 1단계의 출력 결과를 link:prepare-nodes-for-upgrade.html["업그레이드할 노드를 준비합니다"] 섹션에서 node1에 대해 캡처한 출력 결과와 비교하고 올바르게 재배치되지 않은 애그리게이트가 있는지 확인하십시오.
