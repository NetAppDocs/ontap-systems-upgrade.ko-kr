...
== 1단계: node1이 쿼럼 상태인지 확인합니다.

node1이 쿼럼 상태인지 확인합니다. 그런 다음 컨트롤러 교체 작업을 재개합니다.
...
. 이 단계는 다음 업그레이드 구성에 적용됩니다. 다른 모든 시스템 업그레이드의 경우 이 단계를 건너뛰고 다음으로 이동하세요. <<verify-node1-step3,3단계>> :
+
** 2노드 스위치리스 클러스터
** AFF A250 또는 AFF C250 시스템을 AFF A50, AFF A30, AFF C30 또는 AFF C60 시스템으로 업그레이드하는 스위치


+
--
node1이 자동으로 쿼럼에 가입하지 않으면:
...
.. node1 클러스터 LIF를 e1a 및 e1b로 마이그레이션:
+
...
--
. [[verify-node1-step3]]node1과 node2가 동일한 클러스터에 속해 있고 클러스터가 정상인지 확인합니다.
+
`cluster show`
...
. 컨트롤러 교체가 일시 중지된 상태에서 <<restore_network_configuration_on_node1,노드 1에서 네트워크 구성을 복원합니다>>로 진행하십시오.




== 2단계: node1의 네트워크 구성을 복원합니다
...
.이 작업에 대해
VLAN, 인터페이스 그룹 및 브로드캐스트 도메인 생성 및 재생성에 대한 자세한 내용은 link:other-references.html["참조"] _Network Management_ 콘텐츠 링크를 참조하십시오.
...
.다음 단계
노드1 설치를 확인한 후 link:restore-key-manager-config-upgraded-node1.html["업그레이드된 node1에서 키 관리자 구성을 복원합니다"].
