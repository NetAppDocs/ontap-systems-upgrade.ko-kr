...
summary: ARL을 사용하여 동일한 섀시에서 컨트롤러 모델을 업그레이드할 때 해당 교체 시스템 모듈을 교체한 후 node2를 netboot해야 합니다. 
---
= netboot 노드2
:hardbreaks:
:allow-uri-read: 
...
.다음 단계
노드2를 네트워크 부팅한 후 link:cable-node2-for-shared-cluster-HA-storage.html["공유 cluster-HA 및 스토리지용 케이블 node2"].
