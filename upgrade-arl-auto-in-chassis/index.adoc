...
* 다운타임이 포함된 기존 시스템에서 업그레이드하는 경우link:decide_to_use_the_aggregate_relocation_guide.html#supported-systems-in-chassis["지원되는 시스템 매트릭스"] 저장소를 이동하거나 기술 지원팀에 문의하여 컨트롤러 하드웨어를 업그레이드할 수 있습니다.  참조하다link:other_references.html["참조"] _볼륨이나 저장소를 이동하여 업그레이드_에 연결합니다.
