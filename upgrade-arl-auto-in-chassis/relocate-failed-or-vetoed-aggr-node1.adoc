...
summary: 'ONTAP 9.15.1 이상을 실행하는 컨트롤러를 업그레이드할 때 실패하거나 거부된 집계를 수동으로 node1로 다시 배치합니다. `system controller replace` 명령.' 
---
= 실패하거나 거부된 집계를 node1로 다시 이동합니다.
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
집계가 이전되지 않거나 거부된 경우 집계를 node1로 수동으로 이전하거나 필요한 경우 거부 또는 대상 확인을 재정의해야 합니다.
...
.다음 단계
실패했거나 거부된 애그리게이트를 node1로 재배치한 후 link:retire-node2.html["node2를 폐기합니다"].
