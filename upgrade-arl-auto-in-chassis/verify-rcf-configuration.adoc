...
2노드 스위치가 없는 클러스터 구성으로 업그레이드하는 경우 이 섹션을 건너뛰고 link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["루트 이외의 애그리게이트 및 NAS 데이터 LIF를 노드 2로 다시 이동합니다"]로 이동할 수 있습니다.
...
| `Up` | link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["루트 이외의 애그리게이트 및 NAS 데이터 LIF를 노드 2로 다시 이동합니다"]로 이동합니다. 
...
.다음 단계
RCF 구성을 확인한 후 link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["루트가 아닌 애그리게이트 및 NAS 데이터 LIF를 node2로 다시 이동합니다"].
