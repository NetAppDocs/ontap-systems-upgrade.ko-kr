...
[role="lead"]
교체 모듈이 설치된 Node2가 이제 부팅 준비가 완료되었습니다. 지원되는 교체 모듈 목록은 link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["지원되는 시스템 매트릭스"]에 있습니다.


NOTE: 시스템 모듈을 바꿔 업그레이드할 때만 콘솔 및 관리 연결을 이동할 수 있습니다.

.단계
. [[boot_node2_step1]]NetApp Storage Encryption(NSE) 드라이브가 설치되어 있는 경우 다음 단계를 수행하세요.
+
...
+
절차 초반에 기록해 둔 암호와 백업 정보를 입력하십시오. link:complete-preparation-for-upgrade.html#manage-storage-encryption-using-okm["Onboard Key Manager를 사용하여 스토리지 암호화를 관리합니다"]을 참조하십시오.
...
. 노드가 부팅 메뉴에서 멈추면 다음 명령을 node2에서 실행하여 이전 node2 디스크를 교체 node2에 다시 할당합니다.
+
...
.다음 단계
교체 시스템 모듈로 node2를 부팅한 후 link:verify-node2-installation.html["node2 설치를 확인하십시오"].
