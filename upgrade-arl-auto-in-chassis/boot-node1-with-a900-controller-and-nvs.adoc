...
[role="lead"]
교체 모듈이 설치된 Node1이 이제 부팅 준비가 완료되었습니다. 지원되는 교체 모듈은 link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["지원되는 시스템 매트릭스"]에 나열되어 있습니다.
...
.다음 단계
교체 시스템 모듈로 node1을 부팅한 후 link:verify-node1-installation.html["node1 설치를 확인합니다"].
