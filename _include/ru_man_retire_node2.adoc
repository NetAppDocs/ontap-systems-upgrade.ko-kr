...
| 2노드 클러스터 | 로 이동합니다 ,2단계. 


| 2개 이상의 노드가 있는 클러스터 | 로 이동합니다 ,9단계. 
|===
...
+
'cluster modify -node_node2_-epsilon FALSE'

. 노드 3에서 epsilon 'true'를 표시하여 epsilon을 노드 3으로 전송:
+
'cluster modify -node_node3_-epsilon TRUE
...
. 두 컨트롤러 중 하나에 'system node halt-node_node2_' 명령을 입력하여 노드 2를 중단합니다
