...
[role="lead"]
컨트롤러 하드웨어를 업그레이드하는 데는 여러 가지 애그리게이트 재배치(ARL) 방법이 있습니다. 이 ARL 절차에서는 ONTAP 9.15.1 이상에서 도입된 AFF 및 FAS 스토리지 시스템의 HA 컨트롤러 쌍을 기존 디스크와 디스크에 저장된 데이터를 유지하면서 새 컨트롤러로 업그레이드하는 방법을 설명합니다.


NOTE: 이 절차를 사용하여 MetroCluster FC 또는 IP 구성을 업그레이드할 수 없습니다. MetroCluster 구성을 업그레이드하려면 link:other-references.html["참조"]을 참조하여 _MetroCluster 업그레이드 및 확장 문서_로 이동하십시오.
...
.자동 컨트롤러 업그레이드 프로세스
컨트롤러 업그레이드 과정에서 기존 컨트롤러는 최신 또는 더욱 강력한 플랫폼을 실행하는 다른 컨트롤러로 교체됩니다. 이 문서에서는 컨트롤러 업그레이드 과정을 더욱 간소화하기 위해 자동 네트워크 포트 연결 가능성 검사를 활용하는 부분 자동화 절차에 대한 단계를 제공합니다.

.다음 단계
이 ARL 업그레이드 절차에 대해 알게 된 후 link:decide_to_use_the_aggregate_relocation_guide.html["이 절차를 사용할지 여부를 결정하십시오"].
