...
summary: 'ONTAP 9.15.1 이상을 실행하는 컨트롤러에서 명령을 사용하여 ARL 업그레이드를 위한 4단계 프로세스 시퀀스 `system controller replace` .' 
---
= 4단계 개요
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
4단계에서는 비루트 애그리게이트 및 NAS 데이터 LIF를 노드 2에서 노드 3으로 재배치합니다. 또한 프로시저에서 나중에 사용할 수 있도록 필요한 노드2 정보를 기록한 다음 노드2를 폐기합니다.
