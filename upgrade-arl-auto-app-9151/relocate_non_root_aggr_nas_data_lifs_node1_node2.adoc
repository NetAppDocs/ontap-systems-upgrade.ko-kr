...
summary: 'ONTAP 9.15.1 이상 버전에 도입된 컨트롤러로 업그레이드할 때 `system controller replace` 명령을 사용하여 루트가 아닌 애그리게이트와 NAS 데이터 LIF를 노드1에서 노드2로 이동하고 노드1의 리소스를 노드3으로 이동하십시오.' 
---
...
.다음 단계
node1이 소유한 루트가 아닌 애그리게이트 및 NAS 데이터 LIF를 node2로 재배치한 후 link:relocate-failed-or-vetoed-aggr-node2.html["실패했거나 거부된 애그리게이트를 node2로 재배치합니다"].
