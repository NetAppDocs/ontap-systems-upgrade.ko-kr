...
summary: '필요한 경우 ONTAP 9.15.1 이상 버전에 도입된 컨트롤러로 업그레이드할 때 `system controller replace` 명령을 사용하여 스토리지 암호화를 구성하고, SSL 인증서를 설치하고, 키 관리 서버를 설정하십시오.' 
---
...
.다음 단계
새 컨트롤러 모듈에 스토리지 암호화를 설정한 후 link:set_up_netapp_volume_encryption_new_module.html["새 컨트롤러 모듈에 NetApp Volume 또는 Aggregate Encryption을 설정하십시오"].
