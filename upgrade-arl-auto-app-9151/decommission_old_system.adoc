...
summary: 'NetApp에 ONTAP 9.15.1 이상 버전에 도입된 컨트롤러로 업그레이드한 후 이전 시스템이 더 이상 작동하지 않음을 알리려면 `system controller replace` 명령을 사용하십시오.' 
---
= 기존 시스템을 폐기합니다
:hardbreaks:
:allow-uri-read: 
...
.단계
. link:other-references.html["참조"]를 참조하여 _NetApp Support Site_에 연결하고 로그인하십시오.
...
.다음 단계
기존 시스템을 폐기한 후 link:resume_snapmirror_operations.html["SnapMirror 작업 재개"].
