...
summary: 'ONTAP 9.15.1 이상 버전에 도입된 컨트롤러로 업그레이드할 때는  `system controller replace` 명령을 사용하여 노드를 준비하고 일련의 사전 점검을 실행하십시오.' 
---
= 업그레이드할 노드를 준비합니다
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
컨트롤러 교체 프로세스는 일련의 사전 점검으로 시작됩니다. 또한 절차 후반부에 사용할 원래 노드에 대한 정보도 수집합니다. 사전 점검에서 오류가 발생하면 업그레이드를 계속하기 전에 애그리게이트 소유권을 수정해야 합니다.
...
컨트롤러 교체 프로세스를 시작하고 노드1과 노드2에서 사전 점검을 실행합니다. 필요한 경우 사용 중인 자체 암호화 드라이브의 유형을 확인합니다.
...
====
+
다음 예와 유사한 출력이 표시됩니다. 출력에는 클러스터에서 실행 중인 ONTAP 버전이 표시됩니다.
...
| 이미지 버전 확인 | 교체할 노드가 동일한 버전의 ONTAP를 실행하는지 확인합니다. ONTAP 이미지 버전이 다르면 작업이 실패합니다. 새 노드에는 기존 노드에 설치된 것과 동일한 버전의 ONTAP 9.x가 설치되어 있어야 합니다. 새 노드에 다른 버전의 ONTAP가 설치되어 있는 경우 설치 후 새 컨트롤러를 netboot해야 합니다. ONTAP 업그레이드 방법에 대한 지침은 link:other-references.html["참조"]를 참조하여 _Upgrade ONTAP_로 연결하십시오. 
...
| 디스크 상태 확인 | 교체 대상 노드 중 디스크가 누락되었거나 오류가 발생한 노드가 있는 경우 작업이 실패합니다. 디스크가 누락된 경우 link:other-references.html["참조"]를 참조하여 _CLI를 사용한 디스크 및 집합체 관리_, _CLI를 사용한 논리적 스토리지 관리_ 및 _HA 쌍 관리_에 연결하여 HA 쌍에 대한 스토리지를 구성하십시오. 
...
| AutoSupport 상태 확인 | AutoSupport 알림이 구성되지 않은 경우 작업이 실패합니다. 컨트롤러 교체 절차를 시작하기 전에 AutoSupport를 활성화해야 합니다. 
...
== 2단계: ARL 사전 점검이 실패할 경우 애그리게이트 소유권을 수정합니다
...
노드3과 노드4에서 빌드된 이미지가 NODAR(저장 데이터 암호화 없음) 이미지인지 확인하십시오. 빌드된 이미지가 NODAR 이미지인 경우, DARE(저장 데이터 암호화) 이미지를 설치해야 합니다.

.이 작업에 대해
노드3과 노드4에서 다음 단계를 수행하십시오.
...
systemshell
----
. [[check-build]]빌드가 NODAR 이미지인지 확인합니다.
+
[source, cli]
----
cat /BUILD
----
+
명령 출력에서  `build-type`이(가) NODAR 이미지인지 확인하십시오. 예를 들어,  `DEBUG SIM NODAR`.

+

NOTE:  `build-type`가 NODAR 이미지가 아닌 경우, 해당 빌드는 DARE 이미지입니다.

. 빌드가 NODAR 이미지인 경우 DARE 이미지를 설치하십시오.
+
.. 시스템 셸 및 부팅 메뉴를 종료합니다.
+
[source, cli]
----
halt
----
.. DARE 이미지를 설치하십시오. 자세한 내용은 기술 자료 문서 link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Netbooting_a_FAS_and_AFF_Controller_in_ONTAP_9["ONTAP 9에서 FAS 및 AFF 컨트롤러의 네트워크 부팅"^]를 참조하십시오.




.다음 단계
노드를 준비한 후 link:complete-preparation-for-upgrade.html["업그레이드 준비를 완료합니다"].
