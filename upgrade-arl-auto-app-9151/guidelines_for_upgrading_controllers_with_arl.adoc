...
summary: 'ONTAP 9.15.1 이상 버전에 도입된 컨트롤러로 업그레이드할 수 있는지 `system controller replace` 명령을 사용하여 확인하십시오.' 
---
...
[role="lead"]
ARL을 사용하여 ONTAP 9.15.1 이상에 도입된 컨트롤러 쌍으로 업그레이드할 수 있는지 여부는 플랫폼과 기존 컨트롤러 및 교체 컨트롤러 모두의 구성에 따라 다릅니다.
...
* 기존 시스템에서 정의된 모든 애그리게이트의 크기와 지원하는 디스크 수를 확인하십시오. 그런 다음 애그리게이트 크기와 지원하는 디스크 수를 새 시스템에서 지원하는 애그리게이트 크기 및 디스크 수와 비교하십시오. link:other-references.html["참조"]을 참조하여 이 정보를 확인할 수 있는 _Hardware Universe_로 연결하십시오. 새 시스템에서 지원하는 애그리게이트 크기와 디스크 수는 기존 시스템에서 지원하는 애그리게이트 크기와 디스크 수보다 같거나 커야 합니다.
* 기존 컨트롤러가 교체될 때 새 노드가 기존 노드와 함께 클러스터에 포함될 수 있는지 여부를 클러스터 혼합 규칙에서 검증하십시오. 클러스터 혼합 규칙에 대한 자세한 내용은 link:other-references.html["참조"]_Hardware Universe_ 링크를 참조하십시오.
...
.다음 단계
ARL을 이용한 컨트롤러 업그레이드 지침을 검토한 후link:overview_of_the_arl_upgrade.html["ARL 업그레이드 순서에 대해 알아보십시오"].
