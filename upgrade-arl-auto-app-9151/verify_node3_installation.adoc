...
summary: 'ONTAP 9.15.1 이상에 도입된 컨트롤러로 업그레이드할 때 `system controller replace` 명령을 사용하여 노드1의 물리적 포트가 노드3에 올바르게 매핑되는지 확인하고 노드3의 네트워크 구성을 복원하십시오.' 
---
= 노드3 설치를 확인합니다
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
node1의 물리적 포트가 node3의 물리적 포트에 올바르게 매핑되는지 확인해야 합니다. 이렇게 해야 업그레이드 후 node3이 클러스터의 다른 노드 및 네트워크와 통신할 수 있습니다.
...
노드1의 물리적 포트가 노드3의 물리적 포트에 직접 매핑되지 않는 경우, 다음 섹션 <<restore_network_configuration_on_node3_9151,노드3에서 네트워크 구성을 복원합니다>>을 사용하여 네트워크 연결을 복구해야 합니다.

노드 3을 설치하고 부팅한 후에는 올바르게 설치되었는지 확인해야 합니다. 노드 3이 쿼럼에 참가할 때까지 기다린 다음 재배치 작업을 다시 시작해야 합니다.

절차의 이 시점에서 node3이 쿼럼에 참여하면 작업이 일시 중지됩니다.



[[step-1-verify-that-node3-is-in-quorum]]
== 1단계: node3이 쿼럼 상태인지 확인합니다.

노드3이 쿼럼 상태인지 확인한 후 컨트롤러 교체 작업을 재개하십시오.
...
.다음 단계
node3 설치를 확인한 후 link:restore_key-manager_configuration_node3.html["node3에서 키 관리자 구성을 복원합니다"].
