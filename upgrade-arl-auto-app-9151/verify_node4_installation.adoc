...
summary: 'ONTAP 9.15.1 이상에 도입된 컨트롤러로 업그레이드할 때 `system controller replace` 명령을 사용하여 노드2의 물리적 포트가 노드4에 올바르게 매핑되는지 확인하고 노드4의 네트워크 구성을 복원하십시오.' 
---
= 노드 4 설치를 확인합니다
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
노드2의 물리적 포트가 노드4의 물리적 포트에 올바르게 매핑되는지 확인해야 합니다. 이렇게 해야 업그레이드 후 노드4가 클러스터의 다른 노드 및 네트워크와 통신할 수 있습니다.

.이 작업에 대해
link:other-references.html["참조"]를 참조하여 새 노드의 포트에 대한 정보를 얻으려면 _Hardware Universe_ 링크를 참조하십시오. 이 정보는 이 섹션의 뒷부분에서 사용됩니다.

물리적 포트 레이아웃은 노드 모델에 따라 다를 수 있습니다. 새 노드가 부팅될 때 ONTAP는 자동으로 쿼럼에 진입하기 위해 클러스터 LIF를 호스팅해야 하는 포트를 결정하려고 시도합니다.

노드2의 물리적 포트가 노드4의 물리적 포트에 직접 매핑되지 않는 경우, 다음 섹션 <<restore_network_configuration_on_node4_9151,노드 4에서 네트워크 구성을 복원합니다>>을 사용하여 네트워크 연결을 복구해야 합니다.

노드 4를 설치하고 부팅한 후 올바르게 설치되었는지 확인해야 합니다. 노드 4가 쿼럼에 참가할 때까지 기다린 다음 재배치 작업을 다시 시작해야 합니다.

절차의 이 시점에서 node4가 쿼럼에 참여하면 작업이 일시 중지됩니다.



[[step-1-verify-that-node4-is-in-quorum]]
== 1단계: node4가 쿼럼 상태인지 확인합니다.

node4가 쿼럼 상태인지 확인합니다. 그런 다음 컨트롤러 교체 작업을 재개합니다.
...
. 컨트롤러 교체가 다음 메시지와 함께 개입을 위해 일시 중지됩니다.
+
....
...
.이 작업에 대해
VLAN, 인터페이스 그룹 및 브로드캐스트 도메인 생성 및 재생성에 대한 자세한 내용은 link:other-references.html["참조"]_네트워크 관리_ 링크를 참조하십시오.
...
+
연결성 복구 작업이 수행되는 동안 ONTAP은 포트를 올바른 브로드캐스트 도메인에 배치하려고 시도합니다. 그러나 ONTAP이 포트의 연결성을 확인할 수 없고 해당 포트가 기존 브로드캐스트 도메인에 속하지 않는 경우 ONTAP은 해당 포트에 대해 새로운 브로드캐스트 도메인을 생성합니다.
...
.다음 단계
node4 설치를 확인한 후 link:restore_key-manager_config_node4.html["node4에서 key-manager 구성을 복원합니다"].
