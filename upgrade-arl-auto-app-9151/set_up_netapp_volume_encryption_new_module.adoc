...
summary: '필요한 경우, ONTAP 9.15.1 이상 버전에 도입된 컨트롤러로 업그레이드할 때  `system controller replace` 명령을 사용하여 새 컨트롤러 모듈을 NVE 또는 NAE용으로 구성하십시오.' 
---
...
.다음 단계
새 컨트롤러 모듈에서 NetApp Volume 또는 Aggregate Encryption을 설정한 후 link:decommission_old_system.html["기존 시스템을 폐기합니다"].
