...
summary: 'ONTAP 9.15.1 이상 버전에 도입된 컨트롤러로 업그레이드할 때  `system controller replace` 명령을 사용하여 node2의 루트가 아닌 애그리게이트를 node3으로 재배치하십시오.' 
---
...
.다음 단계
루트가 아닌 애그리게이트 및 NAS 데이터 LIF를 node2에서 node3으로 재배치한 후link:relocate-failed-or-vetoed-aggr-node3.html["실패했거나 거부된 애그리게이트를 node3으로 재배치합니다"].
