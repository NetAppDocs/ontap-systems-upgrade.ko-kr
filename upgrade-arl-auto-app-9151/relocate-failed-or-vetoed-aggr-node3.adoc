...
summary: 'ONTAP 9.15.1 이상에 도입된 컨트롤러로 업그레이드할 때  `system controller replace` 명령을 사용하여 실패했거나 거부된 애그리게이트를 수동으로 node3으로 재배치하십시오.' 
---
...
.다음 단계
실패했거나 거부된 애그리게이트를 node3으로 재배치한 후 link:retire_node2.html["node2를 폐기합니다"].
