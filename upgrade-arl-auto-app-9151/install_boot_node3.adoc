...
summary: 'ONTAP 9.15.1 이상에 도입된 컨트롤러로 업그레이드할 때 `system controller replace` 명령을 사용하여 node3를 설치 및 부팅하고 ONTAP 소프트웨어를 설치합니다.' 
---
= 노드3을 설치하고 부팅합니다
:hardbreaks:
:allow-uri-read: 
...
. node3가 부팅 메뉴에서 멈추면 node3에서 다음 명령을 실행하여 node1 디스크를 node3에 다시 할당합니다.
+
...
. 자동 부팅 중에  `Ctrl-C`를 입력하여  `LOADER>` 프롬프트에서 노드를 중지합니다.
...
.다음 단계
node3를 설치하고 부팅한 후 link:verify_node3_installation.html["node3 설치를 확인하십시오"].
