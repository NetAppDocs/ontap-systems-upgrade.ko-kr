...
summary: '필요한 경우, ONTAP 9.15.1 이상 버전에 도입된 컨트롤러로 업그레이드할 때  `system controller replace` 명령을 사용하여 node3의 키 관리자 구성을 복원하십시오.' 
---
...
.다음 단계
node3에서 키 관리자 구성을 복원한 후 link:move_non-root_aggr_and_nas_data_lifs_node1_from_node2_to_node3.html["node1이 소유한 루트가 아닌 애그리게이트 및 NAS 데이터 LIF를 node2에서 node3으로 이동합니다"].
