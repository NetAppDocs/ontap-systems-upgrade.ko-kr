...
summary: 'ONTAP 9.15.1 이상에 도입된 컨트롤러로 업그레이드한 후 HA 페어를 활성화하고, node3 및 node4의 스토리지 액세스를 확인하고, 애그리게이트 소유권을 확인하려면 `system controller replace` 명령을 사용하십시오.' 
---
...
. [[unset_maxwait_system_commands]] 자체 암호화 드라이브가 구성에 사용되고 있고 변수를 (예: 의) 로 설정한 경우 `kmip.init.maxwait` `off` link:install_boot_node4.html#auto_install4_step24["노드 4, 24단계를 설치 및 부팅합니다"]변수를 설정 해제해야 합니다.
+
...
.다음 단계
새 컨트롤러가 올바르게 설정되었는지 확인한 후link:set_up_storage_encryption_new_module.html["새 컨트롤러 모듈에서 스토리지 암호화를 설정합니다"].
