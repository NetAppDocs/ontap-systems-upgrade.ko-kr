...
summary: 노드 3의 설치를 확인하고 노드 2에서 노드 3으로 애그리게이트를 재배치하려면 먼저 NAS 데이터 LIF를 이동해야 합니다. 
---
...
[role="lead"]
노드 3의 설치를 확인하고 노드 2에서 노드 3으로 애그리게이트를 재배치하려면, 현재 노드 2에 있는 노드 1에 속하는 NAS 데이터 LIF를 노드 2에서 노드 3으로 이동해야 합니다. 또한 노드 3에 SAN LIF가 존재하는지 확인해야 합니다.
...
+
'<Vserver_NAME> - 포트셋 <포트셋_이름> - 포트 이름 <포트_이름>'
