...
summary: '"시스템 컨트롤러 교체" 명령을 사용하여 ONTAP 9.5를 실행하는 컨트롤러를 9.7로 업그레이드할 때 노드 2의 NAS 데이터 LIF를 노드 2에서 노드 3으로 이동합니다.' 
---
...
+
'network interface show-data-protocol_iscsi|fcp_-home-node_node3_'
...
... 포트 세트에서 LIF를 제거합니다.
+
' remove-vserver_Vserver_name_-포트셋_이름_-포트-이름_포트_이름_'
...
+
' add-vserver_Vserver_name_-포트셋_이름_-포트-이름_포트_이름_'
...
.. 두 노드 중 하나에서 다음 명령을 입력하고 출력을 검사하여 LIF가 올바른 포트로 이동되었으며 LIF의 상태가 'Up'인지 확인합니다.
+
