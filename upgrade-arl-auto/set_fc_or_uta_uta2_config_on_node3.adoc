...
summary: 나머지 절차를 완료하기 전에 포트 및 카드 설정을 구성하십시오. 
---
...
...
----
. [[step16]] "디스크를 찾을 수 없음" 메시지와 함께 시스템이 재부팅 루프에 들어갈 경우, 이는 포트가 타겟 모드로 다시 설정되었기 때문에 디스크를 볼 수 없기 때문입니다. 를 계속 진행합니다 <<auto_check3_step17,17단계>> 를 선택합니다 <<auto_check3_step22,22단계>> 를 눌러 이 문제를 해결합니다.
