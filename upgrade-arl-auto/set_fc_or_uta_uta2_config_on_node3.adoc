...
** 노드 3에 UTA/UTA2 카드 또는 UTA/UTA2 온보드 포트가 없는 경우 섹션을 건너뛰십시오  and configure UTA/UTA2 ports on node3. 섹션으로 이동합니다 link:map_ports_node1_node3.html["노드 1의 포트를 노드 3으로 매핑합니다"].
...
** 노드 3에 UTA/UTA2 카드 또는 UTA/UTA2 온보드 포트가 있는 경우 섹션으로 이동하십시오  and configure UTA/UTA2 ports on node3.
** 노드 3에 UTA/UTA2 카드 또는 UTA/UTA2 온보드 포트가 없는 경우 섹션을 건너뛰십시오  and configure UTA/UTA2 ports on node3 그런 다음 resume at resume에서 _Install and boot node3_ 섹션으로 돌아갑니다 link:install_boot_node3.html#step23["23단계"].
...
----
====
. 디스크를 찾을 수 없다는 메시지와 함께 시스템이 재부팅 루프에 들어갈 경우, 이는 포트가 타겟 모드로 다시 재설정되었기 때문에 디스크를 볼 수 없기 때문입니다. 를 계속 진행합니다 ,17단계 를 선택합니다 ,22단계 를 눌러 이 문제를 해결합니다.
