...
summary: System controller replace 명령을 사용하여 ONTAP 9.5를 실행하는 컨트롤러를 9.7로 업그레이드할 때 노드를 준비하고 일련의 사전 점검을 수행한다. 
---
= 업그레이드할 노드를 준비합니다
:hardbreaks:
:allow-uri-read: 
...
NOTE: 이 명령은 'Set-Privilege advanced'라는 고급 권한 수준에서만 실행할 수 있습니다
...
** 네트워크 인터페이스 show-curr-node nodename-role cluster, 인터클러스터, 노드 관리, 클러스터 관리, 데이터
** 네트워크 포트 show-node nodename-type physical
...
** 'volume show-node node_name'입니다
** 네트워크 인터페이스 페일오버 그룹들이 보여줌
** 'Storage array config show-switch_name'을 선택합니다
** 'system license show-owner node_name'입니다
** 스토리지 암호화 디스크 표시
