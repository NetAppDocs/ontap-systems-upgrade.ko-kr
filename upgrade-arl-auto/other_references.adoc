...
summary: '을 사용하여 ONTAP 9.5에서 9.7로 컨트롤러 업그레이드를 지원하는 데 필요한 참조 콘텐츠 및 웹 사이트입니다 `system controller replace` 명령.' 
---
= 참조
:hardbreaks:
:allow-uri-read: 
...
| link:https://docs.netapp.com/us-en/ontap-cli/index.html["ONTAP 9 명령 참조"^] | 지원되는 ONTAP 명령의 구문 및 사용법을 설명합니다. 
...
| link:https://docs.netapp.com/us-en/ontap-systems-upgrade/upgrade-arl-auto-in-chassis/index.html[""system controller replace" 명령을 사용하여 동일한 섀시의 컨트롤러 모델을 업그레이드합니다"^] | 시스템을 중단 없이 업그레이드하여 이전 시스템 섀시 및 디스크를 유지하는 데 필요한 애그리게이트 재배치 절차를 설명합니다. 
