...
CAUTION: 이 단계에서 MetroCluster 구성은 정상적인 상태가 아니므로 해결해야 할 오류가 발생할 수 있습니다. link:verify_health_of_metrocluster_config.html["MetroCluster 구성의 상태를 확인합니다"]을 참조하십시오.
...
.이 작업에 대해
VLAN, 인터페이스 그룹 및 브로드캐스트 도메인 생성 및 재생성에 대한 자세한 내용은 link:other-references.html["참조"]로 이동하여 _Network Management_를 참조하십시오.
...
업그레이드할 시스템에서 NetApp Aggregate Encryption(NAE) 또는 NetApp Volume Encryption(NVE)을 사용하여 볼륨을 암호화하는 경우 암호화 구성을 새 노드와 동기화해야 합니다. 키 관리자를 복원하지 않으면 ARL을 사용하여 노드2 애그리게이트를 노드3에서 노드4로 재배치할 때 암호화된 볼륨이 오프라인 상태가 됩니다.
