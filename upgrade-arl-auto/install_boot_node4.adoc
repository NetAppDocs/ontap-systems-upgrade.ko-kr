...
. [[auto_install4_step27]]NetApp Storage Encryption(NSE) 드라이브가 설치되어 있는 경우 다음 단계를 수행하세요.
+
...
+
FC 또는 UTA/UTA2 구성이 없는 경우 노드 4에서 노드 2의 디스크를 인식할 수 있도록 하십시오.link:reassign-node2-disks-to-node4.html#reassign-node2-node4-step1["노드2 디스크를 노드4, 1단계에 재할당합니다"]

. [[29단계]] 스토리지 어레이에 연결된 MetroCluster 구성, V-Series 시스템 및 FlexArray 가상화 소프트웨어가 있는 시스템은 로 이동하십시오.link:reassign-node2-disks-to-node4.html#reassign-node2-node4-step1["노드2 디스크를 노드4, 1단계에 재할당합니다"]
