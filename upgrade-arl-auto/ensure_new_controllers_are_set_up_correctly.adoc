...
summary: '을 사용하여 ONTAP 9.5를 실행하는 컨트롤러를 업그레이드한 후 HA 쌍을 설정하고, 노드 3과 노드 4 스토리지 액세스를 확인하고, 애그리게이트 소유권을 확인합니다 `system controller replace` 명령.' 
---
...
. [[unset_maxwait_system_commands]]구성에서 자체 암호화 드라이브가 사용되고 있고 다음을 설정한 경우 `kmip.init.maxwait` 변수에 `off` (예를 들어,link:install_boot_node4.html#auto_install4_step22["node4 설치 및 부팅, 22단계"] ), 변수를 설정 해제해야 합니다.
+
