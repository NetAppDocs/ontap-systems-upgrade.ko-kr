...
따라서 노드2의 물리적 포트가 노드4의 물리적 포트에 직접 매핑되지 않는 경우 부팅 후 클러스터, 관리 및 네트워크 연결을 복원하려면 소프트웨어 구성 변경이 필요합니다. 또한 노드2의 클러스터 포트가 노드4의 클러스터 포트에 직접 매핑되지 않으면 클러스터 LIF를 올바른 물리적 포트에서 호스팅하도록 소프트웨어 구성을 변경하기 전까지는 노드4가 재부팅 시 쿼럼에 자동으로 다시 참여하지 못할 수 있습니다.
...
+
link:other-references.html["참조"]를 참조하여 _Network Management_ 또는 _ONTAP 9 Command reference_에 연결하고 자세한 내용은 _Configuring failover settings on a LIF_를 참조하십시오.
