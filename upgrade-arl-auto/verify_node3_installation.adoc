...
업그레이드할 시스템에서 볼륨을 암호화하기 위해 NetApp Aggregate Encryption(NAE) 또는 NetApp Volume Encryption(NVE)을 사용하는 경우 암호화 구성을 새 노드와 동기화해야 합니다. 키 관리자를 복원하지 않으면 ARL을 사용하여 node1 애그리게이트를 node2에서 node3으로 재배치할 때 암호화된 볼륨이 오프라인 상태가 됩니다.
