...
summary: 루트가 아닌 애그리게이트 및 NAS 데이터 LIF를 노드 1에서 노드 2로 이동합니다 
---
...
+
'network interface modify -vserver_Vserver_NAME -lif_LIF_name_''' -home-node_nodename_-status-admin up
...
+
'network port vlan delete-node_nodename_-port_ifgrp_-vlan-id_vlan_ID_'
