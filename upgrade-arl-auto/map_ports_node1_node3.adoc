...
[role="lead"]
노드1의 물리적 포트가 노드3의 물리적 포트에 올바르게 매핑되는지 확인해야 합니다. 이렇게 해야 업그레이드 후 노드3이 클러스터의 다른 노드 및 네트워크와 통신할 수 있습니다.

.이 작업에 대해
link:other-references.html["참조"]를 참조하여 새 노드의 포트에 대한 정보를 얻으려면 _Hardware Universe_ 링크를 참조하십시오. 이 정보는 이 섹션의 뒷부분에서 사용됩니다.

노드 모델에 따라 포트 설정이 다를 수 있습니다. 기존 노드의 포트 및 LIF 구성을 새 노드의 계획된 사용 및 구성과 호환되도록 설정해야 합니다. 새 노드는 부팅 시 동일한 구성을 재생성하기 때문입니다. 즉, 노드3을 부팅하면 ONTAP는 노드1에서 사용했던 것과 동일한 포트에서 LIF를 호스팅하려고 시도합니다.

따라서 node1의 물리적 포트가 node3의 물리적 포트에 직접 매핑되지 않는 경우 부팅 후 클러스터, 관리 및 네트워크 연결을 복원하려면 소프트웨어 구성 변경이 필요합니다. 또한 node1의 클러스터 포트가 node3의 클러스터 포트에 직접 매핑되지 않으면 올바른 물리적 포트에서 클러스터 LIF를 호스팅하도록 소프트웨어 구성을 변경하기 전까지 node3이 재부팅 시 쿼럼에 자동으로 다시 참여하지 않을 수 있습니다.
...
+
자세한 내용은 link:other-references.html["참조"]를 참조하여 _Network Management_ 또는 _ONTAP 9 Command reference_에 연결하십시오.
