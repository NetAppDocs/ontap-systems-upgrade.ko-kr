...
---
= AFF, ASA 또는 FAS 컨트롤러 하드웨어 업그레이드 절차 선택
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
일반적으로 NetApp AFF, ASA 또는 FAS 컨트롤러 하드웨어를 업그레이드하는 방법은 기존 노드의 플랫폼 모델에 따라 다릅니다. 업그레이드는 어그리게이트 재배치 또는 볼륨 이동(모두 서비스 중단 없는 절차) 또는 스토리지 이동(서비스 중단이 필요한 절차)을 통해 수행할 수 있습니다. 기존 노드와 새 노드에서 서로 다른 ONTAP 버전이 실행 중인 경우 하드웨어 업그레이드를 시작하기 전에 소프트웨어 업그레이드를 수행해야 할 수도 있습니다.
...
| 원래 노드와 새 노드에서 서로 다른 소프트웨어 버전을 실행하고 있나요? | 원래 노드의 ONTAP 버전을 지원되는 최고 버전으로 업그레이드하십시오. 가능하면 새 노드는 원래 노드에서 실행 중인 ONTAP 버전을 지원해야 합니다. 이것이 불가능한 경우 원래 노드를 link:https://docs.netapp.com/us-en/ontap/upgrade/prepare.html["ONTAP 버전을 업그레이드합니다"^] 지원되는 최고 버전으로 업그레이드하십시오. 원래 노드와 새 노드 간의 버전 차이는 4를 초과할 수 없습니다. 예를 들어 ONTAP 9.8과 9.12.1은 지원되지만 ONTAP 9.8과 9.13.1은 지원되지 않습니다. link:https://docs.netapp.com/us-en/ontap/upgrade/concept_mixed_version_requirements.html["혼합 버전 ONTAP 클러스터에 대해 자세히 알아보십시오"^]. 


| 방해가 되는 업그레이드를 하고 싶은가요? | link:upgrade/upgrade-decide-to-use-this-guide.html["스토리지를 이동하여 업그레이드"]. 이를 위해서는 기존 시스템의 데이터를 저장할 자체 스토리지 및 셸프를 갖춘 새로운 시스템이 필요합니다. 특정 업그레이드의 경우 컨트롤러 셸프에서 변환할 수 있습니다. 
...
* 기존 시스템을 드라이브 셸프로 변환하고 스토리지를 이동합니다.
+
** link:upgrade/upgrade_aff_a250_to_aff_a400_ndu_upgrade_workflow.html["AFF A250을 드라이브 쉘프로 변환하고 스토리지를 AFF A400으로 이동"]
** link:upgrade/convert-to-drive-shelf.html["AFF A150, AFF A220 또는 FAS2820을 드라이브 셸프로 변환하고 지원되는 대체 시스템으로 스토리지를 이동하십시오"]
...
| 모듈형 시스템을 사용하고 있습니까? *참고*: 모듈형 시스템에서는 컨트롤러에 내부 디스크가 없으며 모든 스토리지는 외부에 연결된 셸프에 있습니다. | link:upgrade-arl/index.html["애그리게이트 재배치를 사용하여 업그레이드"]. 
...
| 통합 시스템이 있는데, 기존 섀시와 디스크는 그대로 두고 컨트롤러 모듈만 교체하여 업그레이드하고 싶습니까? | link:upgrade-arl-auto-in-chassis/index.html["ARL을 사용하여 업그레이드하면 동일한 섀시, 디스크 및 디스크에 저장된 데이터를 그대로 유지할 수 있습니다"]. 
...
이 예에서는 FAS9500에서 FAS90 컨트롤러 업그레이드에 대한 가능한 답변을 제공합니다.
