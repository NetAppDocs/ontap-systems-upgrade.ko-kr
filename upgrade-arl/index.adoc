...
summary: AFF, ASA 또는 FAS 컨트롤러 하드웨어를 업그레이드하기 위해 애그리게이트 재배치를 사용하는 방법에 대해 알아봅니다. 
---
= 여기에서 시작하십시오: AFF, ASA 또는 FAS 컨트롤러 하드웨어에 대한 ARL 업그레이드 절차 선택
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
ARL(Aggregate Relocation)을 사용하면 중단 없이 AFF, ASA 또는 FAS 컨트롤러 하드웨어를 업그레이드할 수 있습니다. 컨트롤러 하드웨어를 업그레이드하는 다른 방법은 link:../upgrade/upgrade-decide-to-use-this-guide.html["볼륨 또는 스토리지를 이동하여 업그레이드"]을 참조하십시오.
...
== ONTAP 9.15.1 이상에 도입된 시스템으로 업그레이드

업그레이드 시나리오가 다음 지원 시스템 목록에 있는 경우, link:../upgrade-arl-auto-app-9151/index.html["ONTAP 9.15.1 이상에 도입된 컨트롤러 하드웨어로 업그레이드하려면 "system controller replace" 명령을 사용하십시오."]로 이동하여 업그레이드 절차를 시작하십시오.
...
| AFF A70 | AFF A90 를 참조하십시오 | 9.18.1 이상 


| AFF A30 | AFF A50 를 참조하십시오 | 9.18.1 이상 


| AFF A20 를 참조하십시오 | AFF A30, AFF A50 | 9.18.1 이상 


| AFF C30 | AFF C60 | 9.18.1 이상 


| FAS70 | FAS90 | 9.18.1 이상 
