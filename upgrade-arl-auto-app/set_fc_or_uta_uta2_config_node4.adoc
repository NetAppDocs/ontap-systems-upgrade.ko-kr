...
summary: ONTAP 9.8 이상 버전을 실행하는 컨트롤러를 업그레이드할 때 시스템 컨트롤러 교체 명령을 사용하여 노드4 온보드 FC 포트, UTA 및 UTA2 포트 또는 UTA 및 UTA2 카드를 구성하십시오. 
---
...
.이 작업에 대해
<<configure-fc-ports-on-node4,노드 4에서 FC 포트를 구성합니다>> 또는 <<check-config-uta-uta2-node4,노드 4의 UTA/UTA2 포트를 확인하고 구성합니다>> 섹션 또는 두 섹션 모두를 완료해야 할 수도 있습니다.

[NOTE]
====
노드4에 온보드 FC 포트, 온보드 UTA/UTA2 포트 또는 UTA/UTA2 카드가 없는 경우(예: ONTAP 9.15.1 이상에서 도입된 AFF 및 FAS 시스템) 스토리지 디스크를 사용하여 시스템을 업그레이드하는 경우 link:reassign-node2-disks-to-node4.html["노드2 디스크를 노드4에 재할당합니다"]로 건너뛸 수 있습니다.
...
.이 작업에 대해
시스템에 FC 구성이 없는 경우 이 섹션을 건너뛸 수 있습니다. 시스템에 온보드 UTA/UTA2 포트 또는 UTA/UTA2 어댑터가 있는 경우 <<check-config-uta-uta2-node4,노드 4의 UTA/UTA2 포트를 확인하고 구성합니다>>에서 구성합니다.
...
+
** node4에 UTA/UTA2 카드 또는 UTA/UTA2 온보드 포트가 있는 경우 <<check-config-uta-uta2-node4,노드 4의 UTA/UTA2 포트를 확인하고 구성합니다>>(으)로 이동하십시오.
** node4에 UTA/UTA2 카드나 UTA/UTA2 온보드 포트가 없는 경우 _node4에서 UTA/UTA2 포트 확인 및 구성_을 건너뛰고 다음으로 이동합니다.link:reassign-node2-disks-to-node4.html["노드2 디스크를 노드4에 재할당합니다"] .
...
== 2단계: node4에서 UTA/UTA2 포트를 확인하고 구성합니다
...
.다음 단계
* AFF A800 시스템으로 업그레이드하려면 로 이동하십시오link:reassign-node2-disks-to-node4.html#reassign-node2-node4-app-step9["노드 2 디스크를 노드 4의 9단계에 재할당합니다"].
