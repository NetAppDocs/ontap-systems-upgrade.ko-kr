...
+
MetroCluster IP 구성 업그레이드에 대한 자세한 내용은 link:other-references.html["참조"]_MetroCluster 업그레이드 및 확장_ 링크를 참조하십시오.



[NOTE]
====
* AFF A800 또는 AFF C800과 같이 동일한 섀시에서 컨트롤러 모듈을 교체하여 시스템을 업그레이드하는 경우, NetApp에서는 link:../upgrade-arl-auto-in-chassis/index.html["ARL을 사용하여 컨트롤러 모델을 업그레이드하고 기존 시스템 섀시, 디스크 및 데이터를 유지합니다."] 업그레이드 절차를 사용할 것을 강력히 권장합니다. 이 ARL 절차에는 업그레이드 절차 중 컨트롤러를 제거하고 설치할 때 내부 디스크가 섀시에 안전하게 유지되도록 하는 단계가 포함되어 있습니다.
+
link:../upgrade-arl-auto-in-chassis/decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["ARL을 사용하여 기존 시스템 섀시, 디스크 및 데이터를 유지하면서 지원되는 시스템 업그레이드 조합에 대해 알아보십시오."].
...
^4^다음 표는 컨트롤러 업그레이드 조합에 대해 지원되는 ONTAP 버전을 보여줍니다.
...
| FAS2720 | 9.13.1 이상 | FAS2820 | 9.13.1 이상 


| FAS2760 | 9.11.1P7 이하 | FAS2820 | 9.13.1 이상 
...
.다음 단계
이 ARL 절차를 사용할지 여부를 결정한 후 link:required_tools_and_documentation.html["필수 도구 및 문서"]에 대해 알아보십시오.
