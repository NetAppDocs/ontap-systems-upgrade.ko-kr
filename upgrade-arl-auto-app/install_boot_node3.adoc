...
* 스토리지 디스크를 사용하여 시스템을 업그레이드하는 경우 이 섹션 전체를 완료한 다음 link:set_fc_or_uta_uta2_config_on_node3.html#configure-fc-ports-on-node3["노드 3에서 FC 포트를 구성합니다"] 및 link:set_fc_or_uta_uta2_config_on_node3.html#check-config-uta-uta2-node3["노드 3의 UTA/UTA2 포트를 확인하고 구성합니다"] 섹션으로 이동하여 클러스터 프롬프트에서 명령을 입력해야 합니다.
...
NOTE: 두 노드가 동일한 섀시에 있는 시스템으로 업그레이드하는 경우, 노드4와 노드3을 섀시에 설치하십시오. 두 노드를 동일한 섀시에 설치하지 않으면 노드 3을 부팅할 때 듀얼 섀시 구성인 것처럼 동작하고, 노드 4를 부팅할 때 노드 간 상호 연결이 활성화되지 않습니다.
...
+
link:other-references.html["참조"]를 참조하여 _HA 페어 관리_ 에 연결하십시오.
...
+
절차 초반에 기록해 둔 암호와 백업 정보를 입력하십시오. link:complete-preparation-for-upgrade.html#manage_storage_encryption_using_okm["Onboard Key Manager를 사용하여 스토리지 암호화를 관리합니다"]을 참조하십시오.
