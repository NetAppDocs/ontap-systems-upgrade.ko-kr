...
노드1의 물리적 포트가 노드3의 물리적 포트에 직접 매핑되지 않는 경우, 다음 섹션 <<restore_network_configuration_on_node3,노드3에서 네트워크 구성을 복원합니다>>을 사용하여 네트워크 연결을 복구해야 합니다.
...
+
연결성 복구 작업이 수행되는 동안 ONTAP은 포트를 올바른 브로드캐스트 도메인에 배치하려고 시도합니다. 그러나 ONTAP이 포트의 연결성을 확인할 수 없고 해당 포트가 기존 브로드캐스트 도메인 중 어느 곳에도 속하지 않는 경우, ONTAP은 해당 포트에 대해 새로운 브로드캐스트 도메인을 생성합니다.
...
.다음 단계
node3 설치를 확인한 후 link:restore_key-manager_configuration_node3.html["노드3에서 키 관리자 구성을 복원합니다"].
