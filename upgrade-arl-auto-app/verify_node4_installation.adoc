...
노드2의 물리적 포트가 노드4의 물리적 포트에 직접 매핑되지 않는 경우, <<restore_network_configuration_on_node4,노드 4에서 네트워크 구성을 복원합니다>> 다음 섹션을 사용하여 네트워크 연결을 복구해야 합니다.
...
노드 4가 쿼럼 상태인지 확인한 후 컨트롤러 교체 작업을 재개하십시오.
...
.다음 단계
node4 설치를 확인한 후 link:restore_key-manager_config_node4.html["노드4에서 키 관리자 구성을 복원합니다"].
