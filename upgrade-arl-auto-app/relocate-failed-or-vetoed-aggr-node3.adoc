...
summary: 'ONTAP 9.8 이상을 실행하는 컨트롤러를 업그레이드할 때 실패하거나 거부된 집계를 수동으로 node3로 다시 이동하려면 다음을 사용합니다. `system controller replace` 명령.' 
---
...
.다음 단계
실패했거나 거부된 애그리게이트를 재배치한 후 link:retire_node2.html["node2를 폐기합니다"].
