...
* 노드 3에 온보드 FC 포트, 온보드 UTA/UTA2 포트 또는 UTA/UTA2 카드가 없는 경우 스토리지 디스크로 시스템을 업그레이드하려면 으로 건너뛸 수 있습니다.link:reassign-node1-disks-to-node3.html["노드1 디스크를 노드3에 재할당합니다"]
* 그러나 V-Series 시스템 또는 스토리지 어레이가 포함된 FlexArray 가상화 소프트웨어를 사용하는 시스템이 있고 노드 3에 온보드 FC 포트, 온보드 UTA/UTA 포트 또는 UTA/UTA2 카드가 없는 경우 _ 설치 및 부팅 노드3_로 돌아가서 에서 절차를 재개합니다.link:install_boot_node3.html#auto_install3_step23["23단계"]
...
** 노드 3에 UTA/UTA2 카드 또는 UTA/UTA2 온보드 포트가 없는 경우, skip_check 및 node3_에서 UTA/UTA2 포트를 구성하고 로 이동합니다.link:reassign-node1-disks-to-node3.html["노드1 디스크를 노드3에 재할당합니다"]
...
| V-Series 시스템이거나 FlexArray 가상화 소프트웨어가 있으며 스토리지 어레이에 연결되어 있습니다  a| 
** 노드 3에 UTA/UTA2 카드 또는 UTA/UTA2 온보드 포트가 있는 경우 로 이동하십시오 <<노드 3의 UTA/UTA2 포트를 확인하고 구성합니다>>.
** 노드3에 UTA/UTA2 카드 또는 UTA/UTA2 온보드 포트가 없는 경우, 노드3_에서 UTA/UTA2 포트를 건너뛰고 구성한 후 _설치 및 노드3_로 돌아가서 의 절차를 재개하십시오.link:install_boot_node3.html#auto_install3_step23["23단계"]
...
| V-Series 시스템이거나 FlexArray 가상화 소프트웨어가 있으며 스토리지 어레이에 연결되어 있습니다 | 설치 및 부팅 노드3_로 돌아가서 절차를 다시 link:install_boot_node3.html#auto_install3_step23["23단계"]시작합니다. 
|===
. [[auto_check3_step13]]유지보수 모드 종료:
+
`halt`

. [[14단계]] 를 실행하여 노드를 부팅 메뉴로 부팅합니다. `boot_ontap menu` AFF A800 시스템으로 업그레이드하려면 로 이동하십시오link:reassign-node1-disks-to-node3.html#reassign-node1-node3-app-step9["노드1 디스크를 노드3, 9단계에 재할당합니다"].
