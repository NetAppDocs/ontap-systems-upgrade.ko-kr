...
+
출력에는 애그리게이트가 재배치된 후 `Done`가 표시됩니다.
...
link:other_references.html["참조"]를 참조하여 _CLI를 사용한 디스크 및 애그리게이트 관리_ 콘텐츠 및 _ONTAP 9 명령 참조_에 연결하여 스토리지 애그리게이트 재배치 명령에 대한 자세한 내용을 확인하십시오.
...
.다음 단계
루트가 아닌 애그리게이트를 node1에서 node2로 재배치한 후link:move_nas_lifs_node1_node2.html["node1이 소유한 NAS 데이터 LIF를 node2로 이동합니다"].
