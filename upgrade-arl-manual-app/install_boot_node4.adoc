...
하지만 노드4의 ONTAP 버전이 노드2의 ONTAP 버전과 같거나 그 이상인 경우에는 노드4를 네트워크 부팅할 필요가 없습니다.
...
+
 `disk reassign` 명령은  `_node2_sysid_`가 현재 소유자인 디스크만 재할당합니다.
...
.다음 단계
node4를 설치하고 부팅한 후link:set_fc_uta_uta2_config_node4.html["node4에서 FC 또는 UTA/UTA2 구성을 설정합니다."].
