...
스토리지 애그리게이트 재배치 명령에 대한 자세한 내용은 link:other_references.html["참조"]를 참조하여 _CLI를 사용한 디스크 및 애그리게이트 관리_ 및 _ONTAP 9 명령 참조_에 연결하십시오.
...
.다음 단계
노드2의 루트가 아닌 애그리게이트를 노드3에서 노드4로 재배치한 후link:manage_authentication_kmip.html["KMIP 서버를 사용하여 인증 관리"].
