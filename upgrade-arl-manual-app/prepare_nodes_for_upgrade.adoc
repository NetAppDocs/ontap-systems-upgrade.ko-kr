...
summary: ONTAP 9.8 이상을 실행하는 컨트롤러의 수동 ARL 업그레이드 중에 노드를 준비하고, 정보를 수집하고, 모든 클러스터 노드가 쿼럼에 있는지 확인합니다. 
---
= 업그레이드할 노드를 준비합니다
:hardbreaks:
:allow-uri-read: 
...
+
link:other-references.html["참조"]_HA 페어 관리_ 링크를 참조하고 _HA 페어 모범 사례_ 섹션을 따르십시오. 기존 노드 중 어느 하나라도 사용률이 50%를 초과해서는 안 됩니다. 노드 사용률이 50% 미만인 경우 컨트롤러 업그레이드 중에 두 노드의 부하를 모두 처리할 수 있습니다.
...
사용자 계정 잠금 해제에 대한 자세한 내용은 link:other-references.html["참조"]_시스템 관리 참조_ 링크를 참조하십시오.

====
.. link:other-references.html["참조"]를 참조하여 _NetApp 지원 사이트_ 에 연결하고 성능 및 통계 수집기(Perfstat Converged)를 다운로드하십시오.
+
...
. link:other-references.html["참조"]를 참조하여 _NetApp 지원 사이트_ 링크를 통해 NetApp 지원 사이트에서 지원 사례를 개설하십시오.
+
이 케이스를 사용하여 업그레이드 중에 발생할 수 있는 문제를 보고할 수 있습니다.

. 노드 3 및 노드 4의 NVMEM 또는 NVRAM 배터리가 충전되었는지 확인하고 충전되지 않은 경우 충전합니다.
+
노드3과 노드4 NVMEM 또는 NVRAM 배터리가 충전되었는지 확인하려면 직접 확인해야 합니다. 노드3 및 노드4 모델의 LED에 대한 정보는 link:other-references.html["참조"]_Hardware Universe_에 대한 링크를 참조하십시오.
...
+
새 노드에는 기존 노드에 설치된 것과 동일한 버전의 ONTAP 9.x가 설치되어 있어야 합니다. 새 노드에 다른 버전의 ONTAP가 설치되어 있는 경우, 설치 후 새 컨트롤러를 네트부팅해야 합니다. ONTAP 업그레이드 방법에 대한 지침은 link:other-references.html["참조"]을 참조하여 _ONTAP 업그레이드_로 연결하십시오.
...
....
+
어느 노드든 부분 기브백 상태인 경우,  `storage failover giveback` 명령을 사용하여 기브백을 수행한 다음,  `storage failover show-giveback` 명령을 사용하여 더 이상 기브백해야 할 애그리게이트가 없는지 확인하십시오. 명령에 대한 자세한 내용은 link:other-references.html["참조"]_HA 페어 관리_ 링크를 참조하십시오.
...
+
node1과 node2 모두 현재 소유자(단, 홈 소유자는 아님)인 애그리게이트를 소유하지 않은 경우, 시스템은 다음 예와 유사한 메시지를 반환합니다.
...
....
+
디스크가 누락된 경우 link:other-references.html["참조"]를 참조하여 _CLI를 사용한 디스크 및 집합체 관리_, _CLI를 사용한 논리적 스토리지 관리_ 및 _HA 쌍 관리_에 연결하여 HA 쌍에 대한 스토리지를 구성하십시오.
...
+
 `network interface revert` 명령에 대한 자세한 내용은 link:other-references.html["참조"]를 참조하여 _ONTAP 9 Command reference_에 연결하십시오.
...
+
디스크에 오류가 발생한 경우, _CLI를 사용한 디스크 및 애그리게이트 관리_의 지침에 따라 해당 디스크를 제거하십시오. (_CLI를 사용한 디스크 및 애그리게이트 관리_ 링크는 link:other-references.html["참조"]을 참조하십시오.)

. 다음 하위 단계를 완료하고 각 명령의 출력을 기록하여 node1 및 node2에 대한 정보를 수집합니다.
+
[NOTE]
====
** 이 정보는 나중에 이 절차의 뒷부분에서 사용합니다.
...
NOTE: 이 정보를 사용하면 노드3 또는 노드4로 옮기고 싶은 부품이나 액세서리를 식별할 수 있습니다.
...
+
VLAN은 물리적 포트를 통해 구성됩니다. 물리적 포트가 변경되면 절차 후반에 VLAN을 다시 생성해야 합니다.
...
+
SVM에 iSCSI 또는 FC가 구성되어 있지 않으면 명령에 다음 예와 유사한 메시지가 표시됩니다.
...
+
SVM에 iSCSI 또는 FC가 구성된 경우 명령은 다음 예와 유사한 메시지를 표시합니다.
...
....
+
클러스터의 노드들은 과반수의 노드가 정상 상태이고 서로 통신할 수 있을 때 쿼럼 상태에 있다고 합니다. 자세한 내용은 link:other-references.html["참조"]_시스템 관리 참조_를 참조하십시오.
...
. [[man_prepare_nodes_step28]]다음 하위 단계를 완료하세요.
+
...
+
원격 관리 장치(이 경우 SP)의 네트워크 매개변수를 기존 시스템에서 새 노드의 SP에 재사용할 수 있습니다. SP에 대한 자세한 내용은 link:other-references.html["참조"]를 참조하여 _System Administration Reference_ 및 _ONTAP 9 Command reference_에 연결하십시오.



. [[man_prepare_nodes_step29]]새 노드가 원래 노드와 동일한 라이선스 기능을 갖도록 하려면 다음 명령을 입력하여 원래 시스템의 클러스터 라이선스를 확인하세요.
+
...
....
. 새 노드에 대한 새 라이선스 키는 _NetApp Support Site_에서 발급받으십시오. _NetApp Support Site_ 링크는 link:other-references.html["참조"]을 참조하십시오.
+
...
AutoSupport를 활성화하려면 _시스템 관리 참조_의 지침을 따르십시오. (_시스템 관리 참조_에 연결하려면 link:other-references.html["참조"]을 참조하십시오.)
...
+
AutoSupport에 대한 자세한 내용은 link:other-references.html["참조"]를 참조하여 _System Administration Reference_ 및 _ONTAP 9 Command reference_에 연결하십시오.
...
.다음 단계
노드 업그레이드 준비를 마친 후link:manage_authentication_okm.html["Onboard Key Manager를 사용하여 인증 키 관리"].
