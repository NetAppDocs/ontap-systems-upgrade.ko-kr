...
스토리지 애그리게이트 재배치 명령에 대한 자세한 내용은 link:other_references.html["참조"]로 이동하여 _CLI를 사용한 디스크 및 애그리게이트 관리_ 및 _ONTAP 9 명령 참조_에 연결하십시오.
...
.다음 단계
루트가 아닌 애그리게이트를 node2에서 node3으로 재배치한 후link:move_nas_lifs_node2_node3.html["node2가 소유한 NAS 데이터 LIF를 node3으로 이동"].
