...
+
캡처한 출력과 출력을 비교할 수 있습니다.link:prepare_nodes_for_upgrade.html#man_prepare_nodes_step29["29단계"] _노드를 업그레이드를 위해 준비합니다_.
...
+
link:other-references.html["참조"]로 이동하여 SP에 대한 정보는 _System Administration Reference_를 참조하고  `system service- processor network modify` 명령에 대한 자세한 정보는 _ONTAP 9 Command reference_를 참조하십시오.

. 새 노드에 스위치가 없는 클러스터를 설정하려면 link:other-references.html["참조"]로 이동하여 _네트워크 지원 사이트_에 연결하고 _2노드 스위치가 없는 클러스터로 전환_의 지침을 따르십시오.
...
.다음 단계
새 컨트롤러가 올바르게 설정되었는지 확인한 후link:set_up_storage_encryption_new_controller.html["새 컨트롤러 모듈에서 스토리지 암호화를 설정합니다"].
