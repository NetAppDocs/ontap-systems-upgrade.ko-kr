...
NOTE: 여러 HA 쌍이 있는 클러스터에서 HA 쌍을 업그레이드하는 경우 epsilon을 컨트롤러 업그레이드가 없는 HA 쌍의 노드로 이동해야 합니다. 예를 들어 HA 쌍 구성 NodeA/NodeB 및 nodeC/noded를 사용하여 클러스터의 NodeA/NodeB를 업그레이드하는 경우 epsilon을 노드 C 또는 noded로 이동해야 합니다.
