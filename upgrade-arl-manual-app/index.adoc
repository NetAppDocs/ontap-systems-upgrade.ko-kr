...
+
** MetroCluster ONTAP 9.7 이하 버전을 실행하는 MetroCluster 구성의 경우, link:other-references.html["참조"]ONTAP 9.7 이하 버전을 실행하는 컨트롤러 하드웨어를 수동으로 업그레이드하기 위한 애그리게이트 재배치 사용_ 링크를 참조하십시오.
** MetroCluster IP 구성 및 Fabric MetroCluster 구성에 대한 추가 업그레이드 옵션은 link:other-references.html["참조"]_MetroCluster 업그레이드 및 확장_ 콘텐츠 링크를 참조하십시오.




.다음 단계
이 ARL 절차에 대해 알게 된 후 link:decide_to_use_arl_guide.html["이 ARL 절차를 사용할지 여부를 결정하십시오"].
