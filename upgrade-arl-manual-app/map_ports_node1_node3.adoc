...
+
모든 포트가 올바르게 구성되어 올바른 브로드캐스트 도메인에 추가되면 'network port reachability show' 명령은 연결된 모든 포트에 대한 도달 가능성 상태를 "ok"로 보고하고 물리적 연결이 없는 포트에 대해 "no-reachability"로 상태를 보고해야 합니다. 이 두 포트가 아닌 다른 상태를 보고하는 포트가 있는 경우 에 설명된 대로 내 상태를 복구합니다 ,3단계.
