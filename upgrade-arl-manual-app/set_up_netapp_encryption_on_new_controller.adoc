...
summary: 필요한 경우 ONTAP 9.8 이상을 실행하는 컨트롤러의 수동 ARL 업그레이드 중에 NVE를 위한 새 컨트롤러 모듈을 구성합니다. 
---
...
| 외부 키 관리자(EKM) | `security key-manager external restore`이 명령을 실행하면 OKM 암호가 필요합니다 
