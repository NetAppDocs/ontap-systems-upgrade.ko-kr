...
NOTE: 시스템에 인터클러스터 LIF가 없을 수 있습니다. 노드 쌍의 한 노드에만 클러스터 관리 LIF가 있습니다. 클러스터 관리 LIF는 _node1 포트 정보 기록_의 link:record_node1_information.html#step1["1단계"] 예제 출력에 표시됩니다.
...
+
브로드캐스트 도메인은 절차 후반부에서 새 컨트롤러의 포트에 매핑해야 합니다.
...
.다음 단계
노드2 정보를 기록한 후 link:retire_node2.html["node2를 폐기합니다"].
