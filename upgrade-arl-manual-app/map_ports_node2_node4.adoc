...
[role="lead"]
노드2의 물리적 포트가 노드4의 물리적 포트에 올바르게 매핑되었는지 확인해야 합니다. 이렇게 해야 업그레이드 후 노드4가 클러스터의 다른 노드 및 네트워크와 통신할 수 있습니다.

.시작하기 전에
새 노드의 포트에 대한 정보는 이미 알고 있어야 합니다. 해당 정보에 접근하려면 link:other-references.html["참조"]_Hardware Universe_ 링크를 참조하십시오. 이 정보는 이 섹션 뒷부분에서 사용됩니다.
...
.. 연결성 복구 작업이 수행되는 동안 ONTAP은 포트를 올바른 브로드캐스트 도메인에 배치하려고 시도합니다. 그러나 ONTAP이 포트의 연결성을 확인할 수 없고 해당 포트가 기존 브로드캐스트 도메인 중 어느 것에도 해당하지 않는 경우, ONTAP은 이러한 포트에 대해 새 브로드캐스트 도메인을 생성하려고 시도합니다. 필요한 경우, 모든 멤버 포트가 인터페이스 그룹의 멤버 포트가 되는 경우 새로 생성된 브로드캐스트 도메인을 삭제할 수 있습니다. 브로드캐스트 도메인 삭제:
+
...
. [[man_map_2_Step5]]ONTAP 9.8부터 ONTAP는 네트워크 포트 도달 가능성 복구 절차 중에 브로드캐스트 도메인 간에 포트가 이동된 경우 LIF의 홈 포트를 자동으로 수정합니다. LIF의 홈 포트가 다른 노드로 이동되었거나 할당되지 않은 경우 해당 LIF는 이동된 LIF로 표시됩니다. 홈 포트가 더 이상 존재하지 않거나 다른 노드로 재배치된 이동된 LIF의 홈 포트를 복원하십시오.
+
...
.다음 단계
노드2에서 노드4로 포트를 매핑한 후link:move_nas_lifs_node2_from_node3_node4_verify_san_lifs_node4.html["노드2가 소유한 NAS 데이터 LIF를 노드3에서 노드4로 이동하고 노드4에서 SAN LIF를 확인합니다"].
