...
.이 작업에 대해
원격 LIF는 업그레이드 절차 중에 SAN LUN으로의 트래픽을 처리합니다. 업그레이드 중 클러스터 또는 서비스 상태를 유지하기 위해 SAN LIF를 이동할 필요는 없습니다. SAN LIF는 새 포트에 매핑해야 하는 경우에만 이동됩니다. 노드 3을 온라인 상태로 전환한 후 LIF가 정상 작동하고 적절한 포트에 있는지 확인해야 합니다.
...
.다음 단계
node1이 소유한 NAS 데이터 LIF를 node2에서 node3으로 이동하고 node3에서 SAN LIF를 확인한 후.
