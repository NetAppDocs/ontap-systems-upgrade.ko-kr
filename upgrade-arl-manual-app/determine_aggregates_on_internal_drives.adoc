...
NOTE: 계속 진행하기 전에 먼저 애그리게이트를 오프라인으로 전환한 다음 내부 디스크 드라이브에서 애그리게이트를 삭제해야 합니다. link:other-references.html["참조"]를 참조하여 애그리게이트 관리에 대한 자세한 내용은 _CLI를 사용한 디스크 및 애그리게이트 관리_ 콘텐츠 링크를 참조하십시오.
...
NOTE: 계속 진행하기 전에 볼륨을 외부 디스크 쉘프로 이동하고, 애그리게이트를 오프라인으로 전환한 다음, 내부 디스크 드라이브에서 애그리게이트를 삭제해야 합니다. link:other-references.html["참조"]를 참조하여 볼륨 이동에 대한 자세한 내용은 _CLI를 사용한 디스크 및 애그리게이트 관리_ 콘텐츠 링크를 참조하십시오.



| 내부 드라이브의 루트 볼륨을 포함합니다 | 이 절차를 계속 진행하지 마십시오. link:other-references.html["참조"]를 참조하여 _NetApp 지원 사이트_ 링크로 이동하고 _볼륨 이동을 통한 클러스터형 Data ONTAP 실행 노드 쌍의 컨트롤러 하드웨어 업그레이드_ 절차를 사용하여 컨트롤러를 업그레이드할 수 있습니다. 


| 내부 드라이브에 루트가 아닌 볼륨을 포함하고 외부 스토리지로 볼륨을 이동할 수 없습니다 | 이 절차를 계속 진행하지 마십시오. 볼륨을 이동하여 클러스터형 Data ONTAP을 실행하는 노드 쌍의 컨트롤러 하드웨어 업그레이드_ 절차를 사용하여 컨트롤러를 업그레이드할 수 있습니다. 이 절차에 액세스할 수 있는 _NetApp Support Site_ 링크는 link:other-references.html["참조"]을 참조하십시오. 
|===


.다음 단계
컨트롤러에 내부 디스크 드라이브에 애그리게이트가 있는지 확인한 후 link:prepare_nodes_for_upgrade.html["업그레이드를 위해 노드를 준비합니다"].
