...
[role="lead"]
NetApp FAS2820 시스템의 무중단 업그레이드를 수행하려면 각 노드를 DS212C 드라이브 셸프로 변환합니다. 그런 다음 드라이브 셸프를 교체 노드에 연결합니다. 이렇게 하면 FAS2820 온보드 스토리지가 교체 시스템으로 이동됩니다.
...
.이 작업에 대해
FAS2820 고가용성(HA) 쌍 컨트롤러는 node1과 node2이며, 교체용 HA 쌍 컨트롤러는 node3과 node4입니다.
...
FAS2820 node2를 드라이브 셸프로 변환하기 전에 node2의 논리 인터페이스(LIF)와 데이터 애그리게이트를 node1로 마이그레이션하십시오.
...
[role="quick-margin-para"]
FAS2820 node2를 DS212C 드라이브 쉘프로 변환한 다음 node4에 연결한 후 node2에서 node4로 드라이브를 재할당합니다.
...
[role="quick-margin-para"]
FAS2820 node2를 DS212C 드라이브 셸프로 변환하고 node4에 연결한 후, 이전에 node2에 속해 있던 드라이브를 node4에 다시 할당하십시오.
...
[role="quick-margin-para"]
FAS2820 node1을 드라이브 쉘프로 변환하기 전에 node1의 데이터 애그리게이트, epsilon 및 LIF를 node4로 마이그레이션하십시오.
...
[role="quick-margin-para"]
FAS2820 node1을 DS212C 드라이브 쉘프로 변환하고 node3에 연결한 후 node1에서 node3로 드라이브를 재할당합니다.
...
[role="quick-margin-para"]
FAS2820 node1을 DS212C 드라이브 셸프로 변환하고 node3에 연결한 후, 이전에 node1에 속해 있던 드라이브를 node3에 다시 할당하십시오.
...
[role="quick-margin-para"]
업그레이드를 완료하려면 node3을 node4에 연결한 다음 node4의 데이터 LIF 및 데이터 애그리게이트를 node3으로 마이그레이션하십시오.
