...
summary: '노드2를 드라이브 셸프로 변환하기 전에 노드2의 논리 인터페이스(LIF)와 데이터 애그리게이트를 노드1로 마이그레이션하십시오.' 
---
...
[role="lead"]
노드2를 드라이브 셸프로 변환하기 전에 AFF A150, AFF A220 또는 FAS2820 노드2의 논리 인터페이스(LIF) 및 데이터 애그리게이트를 노드1로 마이그레이션하십시오.

.시작하기 전에
다음 요구 사항을 충족하는지 확인합니다.

* 가능한 경우, 기존 컨트롤러와 교체 컨트롤러는 동일한 ONTAP 릴리스 및 패치 버전을 사용합니다. 지원되는 ONTAP 릴리스에 대한 자세한 내용은  https://hwu.netapp.com["NetApp Hardware Universe를 참조하십시오"^]를 참조하십시오.
...
.다음 단계
link:convert-node2-to-drive-shelf.html["노드 2를 드라이브 쉘프로 변환한 후 노드 4에 연결합니다"]
