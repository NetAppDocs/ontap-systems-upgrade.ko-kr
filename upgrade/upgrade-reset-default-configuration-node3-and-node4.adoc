...
. 부팅 메뉴 프롬프트가 나타나면  `wipeconfig`을 입력한 다음 Enter 키를 누르십시오.
+
다음과 같은 출력이 표시됩니다.
...
----
+
시스템이  `wipeconfig` 절차를 시작하고 재부팅됩니다. 절차가 완료되면 시스템은 부팅 메뉴로 돌아갑니다.

. 부팅 메뉴에서 8을 입력하여 노드를 재부팅하고, AUTOBOOT 중에 Ctrl-C를 입력하여 LOADER 프롬프트에서 노드를 중지합니다.


.다음 단계
새 노드의 기본 구성을 재설정한 후, link:upgrade-install-new-nodes.html["새 노드를 설치합니다"].
