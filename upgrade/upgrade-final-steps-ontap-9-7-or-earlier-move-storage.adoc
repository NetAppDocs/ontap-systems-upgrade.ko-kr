...
summary: '스토리지를 이동시켜 업그레이드하는 절차를 완료하려면 새 노드에서 사용하지 않는 포트와 LIF를 삭제하고, 스토리지 페일오버 또는 고가용성을 다시 활성화하고, 서비스 프로세서(SP)를 구성하고, 새 라이선스를 설치하고, AutoSupport를 설정해야 합니다. 또한 스토리지 또는 볼륨 암호화를 설정하고 FC 또는 CNA 포트를 구성해야 할 수도 있습니다.' 
---
