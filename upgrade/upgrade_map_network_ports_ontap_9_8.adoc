...
. [[MAP_98_5]] 데이터 LIF를 호스팅하는 물리적 포트의 브로드캐스트 도메인 구성원을 수정합니다. ONTAP 9.7 이하 _ 을(를) 사용하여 네트워크 포트 매핑 에 나와 있는 것처럼 수동으로 이 작업을 수행할 수 있습니다. link:upgrade-map-network-ports-ontap-9-7-or-earlier.html#map_97_7["7단계"]. 권장되는 접근 방식은 다음 5단계, 하위 단계(a)에서 (g)까지 ONTAP 9.8에 도입된 향상된 네트워크 도달 가능성 스캔 및 수리 절차를 사용하는 것입니다.
+
