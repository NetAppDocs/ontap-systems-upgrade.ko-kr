...
+
새 애그리게이트와 볼륨이 이동되는 기존 애그리게이트 모두 동일한 SVM에 있어야 합니다.
...
.다음 단계
애그리게이트를 생성하고 볼륨을 새 노드로 이동한 후, link:upgrade-move-lifs-to-new-nodes.html["SAN 데이터 외 LIF와 클러스터 관리 LIF를 새 노드로 이동합니다."].
