...
summary: 볼륨 이동을 통해 업그레이드하는 경우, 기존 노드를 준비하고 새 노드를 클러스터에 추가합니다. 볼륨을 새 노드로 이동하고 LIF를 구성한 다음 기존 노드를 클러스터에서 제거합니다. 볼륨 이동을 통한 업그레이드는 무중단 절차입니다. 
---
= 볼륨 이동을 통한 업그레이드에 대해 알아보십시오
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.다음 단계
볼륨 이동을 통한 업그레이드에 대해 알게 된 후, link:upgrade-prepare-when-moving-volumes.html["업그레이드 준비"].
