...
.. 연결성 복구 작업이 수행되는 동안 ONTAP은 포트를 올바른 브로드캐스트 도메인에 배치하려고 시도합니다. 그러나 ONTAP이 포트의 연결성을 확인할 수 없고 해당 포트가 기존 브로드캐스트 도메인 중 어느 것에도 해당하지 않는 경우 ONTAP은 이러한 포트에 대해 새 브로드캐스트 도메인을 생성합니다. 필요한 경우, 모든 멤버 포트가 인터페이스 그룹의 멤버 포트가 되는 경우 새로 생성된 브로드캐스트 도메인을 삭제할 수 있습니다. 브로드캐스트 도메인 삭제:
+
...
. [[map_98_7]]ONTAP 9.8부터 ONTAP은 네트워크 포트 도달 가능성 복구 절차 중에 브로드캐스트 도메인 간에 포트가 이동된 경우 LIF의 홈 포트를 자동으로 수정합니다. LIF의 홈 포트가 다른 노드로 이동되었거나 할당되지 않은 경우 해당 LIF는 이동된 LIF로 표시됩니다. 홈 포트가 더 이상 존재하지 않거나 다른 노드로 재배치된 이동된 LIF의 홈 포트를 복원하십시오.
+
