...
.단계
. node3의 전원을 켠 다음 콘솔 터미널에서 즉시 Ctrl-C를 입력하여 LOADER 프롬프트에 액세스합니다.
+
...
+
해당하는 경우 사용자 이름/암호를 입력하거나 Enter를 선택하여 계속합니다.
...
.다음 단계
새 노드를 설정한 후, 선택적으로 link:upgrade-move-internal-drives.html["원래 노드에서 내부 드라이브를 이동합니다."] 또는 link:upgrade-convert-node-to-shelf.html["원래 노드를 드라이브 셸프로 변환합니다."]. 그런 다음, link:upgrade-attach-shelves-reassign-disks.html["스토리지 셸프를 연결하고 디스크 소유권을 다시 할당합니다."].
