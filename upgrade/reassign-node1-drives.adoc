...
summary: AFF A150, AFF A220 또는 FAS2820 node1을 드라이브 쉘프로 변환하고 node3에 연결한 후에는 이전에 node1에 속했던 드라이브를 node3에 다시 할당해야 합니다. 
---
...
[role="lead"]
이전에 AFF A150, AFF A220 또는 FAS2820 node1에 할당되었던 드라이브를 node3에 다시 할당하십시오.

.단계
node3에서 다음 단계를 수행하십시오.
...
.다음 단계
link:migrate-node4-lifs-aggregates.html["노드 4의 LIF 및 데이터 애그리게이트를 노드 3으로 마이그레이션"]
