...
+
해당 노드가 쿼럼 마스터인 경우, 클러스터는 일시적으로 쿼럼을 잃었다가 다시 회복합니다. 이러한 쿼럼 손실은 일시적이며 데이터 작업에는 영향을 미치지 않습니다.
...
. <<disable-ha,2>>에서 <<return-admin,11>>까지의 단계를 반복하여 클러스터에서 페일오버 파트너를 제거합니다.
